...
public class Post {
    ...
    public void validatePostDataAllProduct(){
        restAssuredThat(response -> response.body("data.Name", equalTo("tesdr22")));
        restAssuredThat(response -> response.body("data.Description", equalTo("tesdrbisa2")));
        restAssuredThat(response -> response.body("data.Price", equalTo(6969)));
